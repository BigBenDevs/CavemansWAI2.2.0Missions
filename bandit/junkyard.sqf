...
_baserunover31 = createVehicle ["Land_Fire_barrel_burning", [(_position select 0) + 2, (_position select 1) - 9,-0.02],[], 0, "CAN_COLLIDE"];
	...
	//Troops
	[[(_position select 0) - 2, (_position select 1) - 5, 0],4,"Hard",["Random","AT"],4,"Random","CDF_Soldier_Light","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) - 19, (_position select 1) + 19, 0],4,"Hard","Random",4,"Random","CDF_Soldier_Light","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 17, (_position select 1) + 21, 0],4,"Hard","Random",4,"Random","CDF_Soldier_Light","Random","Hero",_mission] call spawn_group;
	...
	if(_complete) then {
		[_crate,15,5,1,2] call dynamic_crate;
			};
