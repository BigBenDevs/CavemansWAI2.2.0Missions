if(isServer) then {
	...
	private 		["_complete","_dir","_crate_type","_mission","_position","_crate","_baserunover","_baserunover1","_baserunover2","_vehicle","_vehclass"];
...
_baserunover42 = createVehicle ["Land_Fire_barrel_burning",[(_position select 0) - 0.01, (_position select 1) - 0.01,-0.02],[], 0, "CAN_COLLIDE"];
	...
	_baserunover 	= [_baserunover0,_baserunover1,_baserunover2,_baserunover3,_baserunover4,_baserunover5,_baserunover6,_baserunover7,_baserunover8,_baserunover9,_baserunover10,_baserunover11,_baserunover12,_baserunover13,_baserunover14,_baserunover15,_baserunover16,_baserunover17,_baserunover20,_baserunover21,_baserunover22,_baserunover23,_baserunover24,_baserunover25,_baserunover26,_baserunover27,_baserunover28,_baserunover29,_baserunover30,_baserunover31,_baserunover32,_baserunover33,_baserunover34,_baserunover35,_baserunover36,_baserunover37,_baserunover38,_baserunover39,_baserunover40,_baserunover41,_baserunover42];
	...
	//Troops
	[[(_position select 0) - 12, (_position select 1) + 2, 0],4,"Hard",["Random","AT"],4,"Random","TK_GUE_Soldier_MG_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 2, (_position select 1) + 2, 0],4,"Hard","Random",4,"Random","TK_GUE_Soldier_Sniper_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 14, (_position select 1) - 35, 0],4,"Hard","Random",4,"Random","TK_GUE_Soldier_Warlord_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 13, (_position select 1) + 35, 0],4,"Hard","Random",4,"Random","TK_GUE_Soldier_Warlord_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 13, (_position select 1) + 30, 0],4,"Hard",["Random","AT"],4,"Random","TK_GUE_Soldier_Warlord_EP1","Random","Hero",_mission] call spawn_group;
	[[(_position select 0) + 13, (_position select 1) + 20, 0],4,"Hard","Random",4,"Random","TK_GUE_Soldier_Warlord_EP1","Random","Hero",_mission] call spawn_group;

	//LAV25 Patrol
	[[(_position select 0) - 22, (_position select 1) - 56, 0],[(_position select 0) + 22, (_position select 1) + 56, 0],50,2,"LAV25","Hard","Bandit","Bandit",_mission] call vehicle_patrol;
	
	//ParaDrop
	[[(_position select 0),(_position select 1),0],[0,0,0],800,"UH60M_EP1_DZE",6,"Random",["Random","AT"],4,"Random","Hero","Random","Hero",false,_mission] spawn heli_para;
	...
	[[[(_position select 0) + 0.2, (_position select 1) + 25, 0]],"M2StaticMG","Hard","Bandit","Bandit",0,2,"Random","Random",_mission] call spawn_static;
	
	// Spawn Vehicles
	_dir 			= floor(round(random 360));

	_vehclass 		= military_armed 		call BIS_fnc_selectRandom;								// Armed vehicle
	
	_vehicle		= [_vehclass,_position,_mission,false,_dir] call custom_publish;
	
	if(debug_mode) then {
		diag_log format["WAI: [Bandit] armybase spawned a %1",_vehclass];
	};
	...
	_complete = [
		...
		[_baserunover,_vehicle], 																	// cleanup objects
